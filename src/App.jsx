  ...
  return (
    <div className="relative pl-16 py-12 group will-change-transform">
      <svg className="absolute left-0 top-1/2 -translate-y-1/2 w-16 h-24 pointer-events-none overflow-visible">
        <motion.path d="M0 12 L30 12 L45 32 L64 32" fill="none" stroke={showActiveState ? "#FFC107" : "#3F3F46"} strokeWidth="0.5" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: delay + 0.5, duration: 1 }} />
        <motion.circle cx="0" cy="12" r="2" fill={showActiveState ? "#FFC107" : "#27272A"} className="transition-colors duration-300" />
        {showActiveState && isVisible && <motion.circle r="1.5" fill="#FFC107" animate={{ cx: [0, 30, 45, 64], cy: [12, 12, 32, 32] }} transition={{ duration: 1.5 + (Math.random() * 0.5), repeat: Infinity, ease: "linear", delay }} />}
      </svg>
      <motion.a href={app.url} onPointerDown={handleTap} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} initial={{ opacity: 0, x: 20 }} animate={{ opacity: isLaunching ? 0.4 : 1, x: isLaunching ? 10 : 0 }} transition={{ delay, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="block">
        <div className="flex flex-col">
          <div className="flex items-center gap-3 mb-2">
            <span className="text-[9px] font-black text-[#FFC107] uppercase tracking-[0.5em]">Node_{app.node}</span>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center selection:bg-[#FFC107]/30 overflow-hidden touch-none overscroll-none font-['Outfit']">
      ...
      <div className="relative w-full max-w-lg mt-12 pb-60 px-8 h-screen overflow-y-auto no-scrollbar touch-pan-y z-10">
        <div className="flex flex-col gap-4 mb-16 pt-16">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-[#FFC107] uppercase tracking-[0.8em] opacity-60">Lattice Registry</span>
            </div>
            <h1 className="text-5xl font-black tracking-tighter uppercase leading-none text-white selection:text-[#FFC107]">Onyx Hub</h1>
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-[600] flex justify-center pointer-events-none pb-[calc(1.5rem+env(safe-area-inset-bottom,20px))]">
        ...
        >
          {/* Internal Orb Glow */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_100%,rgba(255,193,7,0.1)_0%,transparent_70%)] pointer-events-none" />
          ...
          <AnimatePresence mode="wait">
            {!isOrbExpanded ? (
              <motion.div key="compact" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="w-full h-full flex items-center justify-center relative">
                ...
                <div className="relative w-3 h-3 bg-[#FFC107] rounded-full shadow-[0_0_20px_rgba(255,193,7,0.9)]" />
              </motion.div>
            ) : (
              <motion.div key="expanded" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="w-full h-full p-8 flex flex-col relative z-10">
                <div className="flex justify-between items-start mb-12 shrink-0">
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-[#FFC107] uppercase tracking-[0.5em]">Command</span>
                ...
                <div className="flex-1 flex flex-col gap-12">
                   ...
                   <div className="flex flex-col gap-3 mt-auto">
                      <button onClick={(e) => { e.stopPropagation(); forceRefresh(); }} className="flex items-center justify-between px-6 py-4 rounded-2xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-[0.4em] hover:bg-[#FFC107] hover:text-black transition-all">
                        <span>Sync</span>
